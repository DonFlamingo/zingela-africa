...
function History() {
    ...
    _this.events = function() {
        $(document)
            ...
            .on('shown.bs.tab', '[data-toggle="tab"][href="#history-list"]', function (e) {
                dd('History.shown.bs.tab');
                _this.getHistories();
            })
    ...
    _this.parse = function() {
        ...
        if (window.history_items != null) {
            ...
            if ( app.settings.showHistoryArrow ) {
                polylines.addLayer( L.polylineDecorator(polyArray, {
                    patterns: [
                        {
    ...
    _this.getHistories = function() {
        var $container = $('#messages_tab2');
        ...
        var historiesConfig = {
          url : app.urls.historyHistory,
          delete_url: app.urls.historyPositionsDelete
        };

        tables.get('messages_tab2', historiesConfig);
    };
...
function Devices() {
    ...
    _this.updateData = function( $container ) {
        $.each(items, function (id, device) {
            if (!device)
                return;

            device.dataDOMUpdate( $container );
        });
    };
    ...
    _this.updateStopDuration = function() {
        ...
        $('[data-device="stop_duration"]', $widgets ).replaceWith(device.dataDOM('stop_duration'));
...
function getoffsetLatlng(lat, lon){
	//Position, decimal degrees
	//var lat = 12.894825833333
	//var lon = 77.581492777778
	...
	//console.log(lat +" "+ lon);
	//console.log("new lat lng => "+ latO + " " +lonO );
	return {"lat":latO,"lng":lonO};	
}

function getoffsetLatlng2(lat, lon){
	//Position, decimal degrees
	//var lat = 12.894825833333
	//var lon = 77.581492777778
